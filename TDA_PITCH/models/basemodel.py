...
class BaseModel(pl.LightningModule):
    """Base Lightning Module - DO NOT CREATE DATA MODULE HERE!
    ...
    def configure_callbacks(self):
        ...
        if self.moniter_metric == 'valid_loss':
            checkpoint_filename = '{epoch}-{valid_loss:.4f}'
        # TODO Change this condition to a monitor metric pertinent to my models
        elif self.moniter_metric == 'valid_wer':
            checkpoint_filename = '{epoch}-{valid_wer:.4f}'
        else:
            assert ValueError(f"Monitor_metric {self.monitor_metric} is not valid")
